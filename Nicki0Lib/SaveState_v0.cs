	...
	class SaveState_v0 {
		...
		public static Dictionary<string, List<string>> GetDictionaryListData(WorldObject wo) {
			...
			foreach (string keyAndList in wo.GetText().Split(";")) {
				...
				Assert.IsTrue(keyAndListArray.Count() == 2, nameof(SaveState_v0.GetDictionaryListData).ToString() + ": Wrong data format: there may only be one key and list");
		...
		public static Dictionary<K, T> GetDictionaryData<K, T>(WorldObject wo, Func<string, K> KFunc, Func<string, T> TFunc) {
			...
			foreach (string keyAndValue in text.Split(";")) {
				...
				Assert.IsTrue(keyAndValueArray.Count() == 2, nameof(SaveState_v0.GetDictionaryData).ToString() + ": Wrong data format: there may only be one key and list");
